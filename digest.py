...
import os, json, smtplib, urllib.request, urllib.error, re, time
...
def get_recipients():
    ...
    try:
        with open(BASE_DIR / "recipients.json", encoding="utf-8") as f:
            data = json.load(f)
        r = data.get("digest", [])
        if not r: raise ValueError("Aucun destinataire")
        return r
    except FileNotFoundError:
        raise SystemExit("❌ recipients.json introuvable.")
...
def groq_summarize(context: str, section: dict, lang: str, x_signals: str = "") -> str:
    """
...
"""
    ...
    try:
        ...
        if e.code == 429:
            ...
            time.sleep(60)
            return groq_summarize(context, section, lang, x_signals)
...
def main():
    ...
    if NOTIF_TYPE == "alert":
        ...
        matin = now.hour < 14
        ed    = ("Matin 🌅" if matin else "Soir 🌙") + " / " + ("Mañana 🌅" if matin else "Noche 🌙")
